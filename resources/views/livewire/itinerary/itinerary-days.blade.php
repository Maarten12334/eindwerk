        ...
        <div class="p-6 bg-primaryGreen dark:bg-gray-700 rounded-lg shadow-md">
            ...
            <div class="mt-4">
                <form wire:submit.prevent="addItem">
                    <input type="hidden" wire:model="current_date" value="{{ $current_date->format('Y-m-d') }}">
                    <div class="mb-2">
                    ...
                    <div class="mb-2">
                        ...
                        <input type="time" id="time" wire:model="time" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        @error('time') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                    </div>
                    <button type="submit" class="mt-2 inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:outline-none focus:border-green-700 focus:ring focus:ring-green-200 active:bg-green-700 disabled:opacity-25 transition" wire:click="setCurrentDate('{{ $current_date->format('Y-m-d') }}')">Add Item</button>
                </form>
            </div>
        </div>
    </div>
