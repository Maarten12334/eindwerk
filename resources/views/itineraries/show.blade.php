            ...
            <div class="p-6 text-secondaryGreen dark:text-gray-100">
                ...
                <div class="flex flex-wrap -mx-4">
                    ...
                    <div class="w-full md:w-1/2 px-4 mb-6 {{ $counter % 2 == 0 ? 'md:self-start' : 'md:self-end' }}">
                        ...
                        @endphp
                        <div class="p-6 bg-primaryGreen dark:bg-gray-700 rounded-lg shadow-md {{ $itemsCount > 10 ? 'max-h-96 overflow-y-scroll' : '' }}">
                            <h5 class="text-lg font-semibold mb-4">{{ $current_date->format('d-m') }}</h5>
                            @if ($itemsCount > 0)
                            @if ($itemsCount > 5)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach ($items as $item)
                                <div class="p-2 bg-secondaryGreen dark:bg-gray-600 rounded border border-gray-200 dark:border-gray-500 text-gray-700 dark:text-gray-300 flex justify-between items-center">
                                    <li>
                                        {{ $item->type }} {{$item->time}}
                                    </li>
                                    <div class="ml-4 flex items-center space-x-2">
                                        ...
                                        <button wire:click="deleteItem({{ $item->id }})" class="text-secondaryGreen hover:text-red-700">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <ul class="space-y-2">
                                @foreach ($items as $item)
                                <li class="p-2 bg-secondaryGreen dark:bg-gray-600 rounded border border-gray-200 dark:border-gray-500 text-primaryGreen dark:text-gray-300 flex justify-between items-center">
